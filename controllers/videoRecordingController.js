...
const insertVideoRecURL = async (req, res, next) => {
  try {
    ...
    let source_link, pool;
    // console.log(`video rec url ${video_rec_url}`)
    pool = await Pool.findById(pool_id);
    pool = pool[0][0];
    source_link = pool.source;
    ...
    let videoRecordings = new VideoRecording(
      video_rec_url,
      user_id,
      pool_id,
      source_link
  ...
  try {
    ...
    videoRecordings = await Promise.all(
      videoRecordings.map(async (videoRec) => {
        ...
        let bookmarks = await Bookmarks.getByUserAndVideo(
          req.user.user_id,
          videoRec.video_id
        );

        if (bookmarks != undefined && bookmarks[0].length > 0) {
          videoRec.is_bookmarked = true;
        } else {
          videoRec.is_bookmarked = false;
        }
