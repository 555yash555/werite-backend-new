...
const createPoolRequest = async (req, res, next) => {
  try {
    ...
    if (req.user.merit < poolData.merit_required) {
      return res
        .status(400)
        .json({ message: "You don't have enough merit to join this pool" });
    }
...
const editPoolRequest = async (req, res) => {
  try {
    const user_id = req.user.user_id;
    const pool_id = req.params.id;
    ...
    let updatedReq = {};
    let poolRequest = await PoolRequest.findOne(user_id, pool_id);
    poolRequest = poolRequest[0][0];

    if (poolRequest) {
      updatedReq = await PoolRequest.update(pool_id, user_id, stance, guts);
      updatedReq = updatedReq[0][0];
      return res.status(200).json({ Success: "OK", updatedReq });
    } else {
