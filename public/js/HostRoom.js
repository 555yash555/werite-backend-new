...
//   // Hide the overlay after a delay (e.g., 5 seconds)
//   setTimeout(() => {
//     overlay.style.display = 'none';
...
function showOverlay(message) {
  ...
  overlay.offsetHeight; /* Trigger reflow to restart the animation */
  overlay.style.animation = 'slideIn 5s linear';

  // Show the overlay
  overlay.style.display = 'block';

  // Calculate animation duration based on the length of the message
  const duration = 500 * message.length; // Adjust the multiplier as needed
...
// Example usage
// showOverlay('This is a test message that flows from right to left.');
async function fetchPoolData(poolId) {
  try {
    const response = await fetch(`http://localhost:8080/join/poolDat/${room_id}`, {
      method: "GET",
      headers: {
...
async function displayPromptsWithDelay() {
  
  const poolData = await fetchPoolData(room_id);
  

  if (!poolData) {
    setTimeout(() => {
      // Show the prompt in the overlay
      showOverlay('error fetching pool data');
    }, 5000);
    console.error('Unable to fetch pool data.');
    return;
  }


  // Iterate through each prompt in the pool
  poolData.prompts.forEach((prompt) => {
    const { text, timeMinutes } = prompt;
    ...
    // Set a timeout to display the prompt after the specified time interval
    setTimeout(() => {
      // Show the prompt in the overlay
      showOverlay(text);
    }, timeMilliseconds);
...
function joinRoom(peer_name, room_id) {
  ...
  fetch(`http://localhost:8080/join/${room_id}/1`, {
    method: "PATCH",
    headers: {
      "Content-Type": "application/json",
    },
  ...
  fetch(`http://localhost:8080/update/userincall/${room_id}`, {
    method: "PATCH",
    headers: {
      "Content-Type": "application/json",
      Authorization: token,
  ...
  fetch(`http://localhost:8080/join/poolData/${room_id}`, {
    ...
    .then((data) => {
      ...
      displayPromptsWithDelay(data)

    })
    .catch((err) => {
      console.log("error", err);
    });
    displayPromptsWithDelay();
}
...
function handleButtons() {
  ...
  tabRecordingBtnr.onclick = () => {
    rc.startRecording();
  };
  stopRecButtonn.onclick=()=>{
    rc.stopRecording();
  };
  ...
  // Create an object to store the count of accept and reject clicks for each peer
// let peerVotes = {};

// acceptButton.onclick = () => {
//     let peer_name = prompt("Enter the name of the peer you are accepting");
//     if (peer_name) {
//         rc.updatePeerInfo(peer_name, rc.peer_id, "banner", true);
//         // Update the count of accept clicks for the peer
//         if (!peerVotes[peer_name]) peerVotes[peer_name] = { accept: 0, reject: 0 };
//         peerVotes[peer_name].accept++;
//         updateVoteDisplay(peer_name);
//     }
// };

// rejectButton.onclick = () => {
//     let peer_name = prompt("Enter the name of the peer you are rejecting");
//     if (peer_name) {
//         rc.updatePeerInfo(peer_name, rc.peer_id, "banner", false);
//         // Update the count of reject clicks for the peer
//         if (!peerVotes[peer_name]) peerVotes[peer_name] = { accept: 0, reject: 0 };
//         peerVotes[peer_name].reject++;
//         updateVoteDisplay(peer_name);
//     }
// };

// function updateVoteDisplay(peer_name) {
//     // Calculate the percentage distribution of accept and reject clicks
//     let totalVotes = peerVotes[peer_name].accept + peerVotes[peer_name].reject;
//     let acceptPercentage = (peerVotes[peer_name].accept / totalVotes) * 100;
//     let rejectPercentage = (peerVotes[peer_name].reject / totalVotes) * 100;

//     // Display the count and percentage distribution
//     let voteDisplay = document.getElementById('voteDisplay');
//     voteDisplay.innerHTML = `Votes for ${peer_name}: ${peerVotes[peer_name].accept} accept (${acceptPercentage.toFixed(2)}%), ${peerVotes[peer_name].reject} reject (${rejectPercentage.toFixed(2)}%)`;
// }
// Function to update the visual representation of votes using a progress bar
// function updateVoteBar(peer_name) {
//   let acceptPercentage = (peerVotes[peer_name].accept / (peerVotes[peer_name].accept + peerVotes[peer_name].reject)) * 100;

//   // Update the progress bar elements
//   let acceptProgressBar = document.getElementById('acceptProgressBar');
//   let rejectProgressBar = document.getElementById('rejectProgressBar');

//   acceptProgressBar.style.width = `${acceptPercentage}%`;
//   rejectProgressBar.style.width = `${100 - acceptPercentage}%`;
// }

// Modify the updateVoteDisplay function to call updateVoteBar
// function updateVoteDisplay(peer_name) {
//   // Calculate the percentage distribution of accept and reject clicks
//   let totalVotes = peerVotes[peer_name].accept + peerVotes[peer_name].reject;
//   let acceptPercentage = (peerVotes[peer_name].accept / totalVotes) * 100;
...
function handleRoomClientEvents() {
  ...
  rc.on(RoomClient.EVENTS.accept, () => {
    console.log("ACCEPT");
    // hide(raiseHandButton);
    // show(lowerHandButton);
    // setColor(lowerHandButton, "green");
