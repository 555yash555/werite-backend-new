...
class RoomClient {
  ...
  async handleProducer(id, type, stream) {
    ...
    switch (type) {
      ...
      case mediaType.screen:
        ...
        d.className = "Camera " +this.peer_id+"__video"; 
        d.id = id + "__video";
  ...
  handleConsumer(id, type, stream, peer_name, peer_info) {
    ...
    switch (type) {
      ...
      case mediaType.screen:
        ...
        d.className = "Camera " +remotePeerId+"__video"
        d.id = id + "__video";
        elem = document.createElement("video");
        elem.setAttribute("id", id);
        elem.setAttribute("playsinline", true);
  ...
  async setVideoOff(peer_info, remotePeer = false) {
    let d, vb, i, h, au, sf, sm, sv, ko, p, pm, pb, pv,border;
    ...
    d.className = "Camera " +peer_id+"__video"
  ...
  exitRoom(token, pool_id) {
    ...
    fetch(`https://localhost:8080/update/usernotincall`, {
      method: "PATCH",
      headers: {
      ...
      .then((data) => {
        ...
        fetch(`https://localhost:8080/iscurrentpoolempty/${pool_id}`)
          .then((response) => response.json())
          .then((data) => {
            console.log(data.message);
            if (data.message === "Pool Empty") {
              fetch(`https://localhost:8080/join/${pool_id}/0`, {
                method: "PATCH",
                headers: {
                  "Content-Type": "application/json",
                },
  ...
  getPeerbannerBtn(peer_id) {
    return this.getclass("Camera " +peer_id+"__video");
  }
  ...
  updatePeerInfo(peer_name, peer_id, type, status, emit = true) {
    if (emit) {
      switch (type) {
        ...
        case "banner":
          this.peer_info.peer_banner = status;
          let peer_banner = this.getPeerbannerBtn(peer_id);
          if (status) {
            if (peer_banner) {
                // Set a thicker border with a gradient green color
                peer_banner.style.border = "6px solid";
                peer_banner.style.borderImage = "linear-gradient(to bottom, green, yellow, green) 1";
                ;
                peer_banner.style.borderRadius = "10px"; 
        
                // Create a function to handle the flashing effect
                function flashGreenBorder() {
                    let borderStyle = peer_banner.style.borderImage;
                    peer_banner.style.borderImage = borderStyle ? "" : "linear-gradient(to bottom, green, yellow, green) 1";
                    peer_banner.style.borderRadius = "10px"; 
                }
        
                // Flash the green border twice at 500ms intervals
                let flashCount = 0;
                let flashInterval = setInterval(function () {
                    flashGreenBorder();
                    flashCount++;
                    if (flashCount >= 6) { // Flash twice (2 on, 2 off)
                        clearInterval(flashInterval);
        
                        // Show the gradient border for 1 minute (60,000 milliseconds)
                        setTimeout(() => {
                            peer_banner.style.borderImage = "linear-gradient(to bottom, green, yellow, green) 1";
                            peer_banner.style.borderRadius = "10px"; 
                        }, 2000);
                        ...
                        setTimeout(() => {
                            peer_banner.style.border = "none"; // Remove the border after 1 minute
                        }, 60000);
                    }
                }, 250);
        
                this.event(_EVENTS.accept);
            }
        } else {
            if (peer_banner) {
                // Set a thicker border with a gradient red color
                peer_banner.style.border = "6px solid";
                peer_banner.style.borderImage = "linear-gradient(90deg, red, yellow, red) 1";
                peer_banner.style.borderRadius = "10px"; 
        
                // Create a function to handle the flashing effect
                function flashRedBorder() {
                    let borderStyle = peer_banner.style.borderImage;
                    peer_banner.style.borderImage = borderStyle ? "" : "linear-gradient(90deg, red, yellow, red) 1";
                    peer_banner.style.borderRadius = "10px"; 
                }
        
                // Flash the red border twice at 500ms intervals
                let flashCount = 0;
                let flashInterval = setInterval(function () {
                    flashRedBorder();
                    flashCount++;
                    if (flashCount >= 4) { // Flash twice (2 on, 2 off)
                        clearInterval(flashInterval);
